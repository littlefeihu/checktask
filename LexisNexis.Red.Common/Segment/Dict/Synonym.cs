...
namespace LexisNexis.Red.Common.Segment.Dict
{
    ...
    class Synonym
    {
        ...
        private void LoadSynonym(string fileName)
        {
            ...
            if (! GlobalAccess.DirectoryService.FileExists(fileName).Result)
            {
                return;
            }

            //Stream sr = GlobalAccess.DirectoryService.OpenFile(fileName, FileModeEnum.Open).Result;
            using (Stream s = GlobalAccess.DirectoryService.OpenFile(fileName, FileModeEnum.Open).Result)
            {
