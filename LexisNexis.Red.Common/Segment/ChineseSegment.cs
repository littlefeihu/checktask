...
using LexisNexis.Red.Common.Segment.Dict;
using LexisNexis.Red.Common.Resource;
using LexisNexis.Red.Common.Common;
using System.Threading.Tasks;

namespace LexisNexis.Red.Common.Segment
{
    public class ChineseSegment : ISegment
    {
        ...
        public List<String> Analyse(string text)
        {
            ...
            foreach(WordInfo element in DoSegment(text))
            {
        ...
        public ICollection<WordInfo> DoSegment(string text, Match.MatchOptions options, Match.MatchParameter parameters)
        {
            ...
            try
            {
                ...
                if (_Options == null)
                {
                    _Options = new Match.MatchOptions();
                }

                if (_Parameters == null)
                {
                    _Parameters = new Match.MatchParameter();
                }
            ...
            {
                Logger.Log("Segment Failed : " + ex.Message);
                return null;
            }
            //finally
            //{
        ...
        private static async Task LoadDictionary()
        {
            try
            {
                
               _WordDictionary = new Dict.WordDictionary();
               await _WordDictionary.Load(Constants.DICTIONARY_FILE_NAME, true);
               ...
               //string dirStop = System.IO.Path.Combine(GlobalAccess.DirectoryService.GetAppRootPath(), Constants.STOPWORD_FILE_NAME);
               _StopWord = new Dict.StopWord();
               await _StopWord.LoadStopwordsDict(Constants.STOPWORD_FILE_NAME);
               
            }
            catch (Exception ex)
            {
                Logger.Log("LoadDictionary Failed : " + ex.Message);
            }
            ...
            //_DictLoader = new Dict.DictionaryLoader(Setting.PanGuSettings.Config.GetDictionaryPath());
        }
        ...
        public async Task Init(string fileName)
        {
            ...
            //_Wildcard = new Dict.Wildcard(Setting.PanGuSettings.Config.MatchOptions,
