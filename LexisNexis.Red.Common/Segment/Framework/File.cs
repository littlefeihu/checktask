...
namespace LexisNexis.Red.Common.Segment.Framework
{
    public class File
    {
        ...
        static public String ReadFileToString(String FileName, Encoding encode)
        {
            ...
            while (true)
            {
                try
                {
                    using (System.IO.Stream s = GlobalAccess.DirectoryService.OpenFile(FileName, FileModeEnum.Open).Result)
                    {
                        Stream.ReadStreamToString(s, out str, encode);
                    }
                    return str;
                }
                catch (Exception)
