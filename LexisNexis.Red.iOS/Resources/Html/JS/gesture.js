...
function prepareTarget(target, eventListener) {
    ...
    target.addEventListener("LostPointerCapture", function () {
        ...
        var jsonObj = "{ type:\"MSGestureEnd\", value1:\"" + value + "\"}";
    ...
    target.addEventListener("pointercancel ", onPoint, false);
    target.addEventListener("lostpointercapture ", onPoint, false);
    target.addEventListener("MSLostPointerCapture", onPoint, false);
...
function eventListener(evt) {
    ...
    if (evt.type == "MSGestureStart") {
        ...
        var jsonObj = "{ type:\"MSGestureEnd\", value1:\"" + value + "\"}";
        window.external.notify(jsonObj);
        //evt.target.setPointerCapture(gesture.pointerId);
        //document.getElementById('container').onmousedown;
    }
    else if (evt.detail & evt.MSGESTURE_FLAG_END) {
        var value = window.getSelection();
        var jsonObj = "{ type:\"MSGestureEnd\", value1:\"" + value + "\"}";
        window.external.notify(jsonObj);
    }
    else if (evt.detail & evt.MSGESTURE_FLAG_CANCEL) {
        var value = window.getSelection();
        var jsonObj = "{ type:\"MSGestureEnd\", value1:\"" + value + "\"}";
        window.external.notify(jsonObj);
    }
